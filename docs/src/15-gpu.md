...
!!! warning "Log-density-only models on GPU"
    `grad_logdensity` can itself be an AD backend (`DensityModel(logp, AutoEnzyme(), dim)`, see [Getting started](10-getting-started.md)), but don't do that with `ParallelMALASampler` on GPU.  The HVP is then true second-order AD — `SecondOrder(hvp_backend, grad_backend)` on your log-density — which currently fails on GPU with both Enzyme and Mooncake (see [#37](https://github.com/rsenne/ParallelMCMC.jl/issues/37)).  The same goes for passing a `SecondOrder` explicitly.  Write `gradlogp` out by hand for DEER, so the HVP is a single pass over it.  The sequential samplers only need the gradient, so log-density-only models are fine there.

!!! note "A hand-written gradient keeps AD out of the batched path"
    The batched DEER path needs a batched gradient as well as a `logdensity_batch`.  One is derived from `logdensity_batch` only when `grad_logdensity` is a backend — i.e. only for a model that already opted into AD over its own code.  With a hand-written `grad_logdensity` and no `grad_logdensity_batch`, the batched path simply stays off and the unbatched update runs, so nothing you wrote by hand is ever handed to AD behind your back.  The sampler's `backend` does not change this; it only ever supplies HVPs.
...
- **You're hitting AD-backend-specific GPU restrictions.**  The [Enzyme limitations](#2-enzyme-on-gpu-currently-needs-pmcmc_matmul-pmcmc_dot-pmcmc_dotsum) above (`pmcmc_*` wrappers, staged broadcasts) only matter when the AD backend is invoked.  Supplying analytical HVP sidesteps them — your `gradlogp` and `hvp` can use plain `*`, `dot`, `sum`, and the sampler's `backend` can be omitted entirely because no AD is ever invoked.  A model whose derivatives are all hand-written never reaches AD by any route, batched path included.
