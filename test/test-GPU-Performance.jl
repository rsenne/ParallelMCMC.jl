...
const _PERF_GPU_AVAILABLE = try
    CUDA.functional() && (CUDA.CuArray([1f0]); true)
catch
    false
end
...
function _bench(model, sampler, N; x0, reps=2)
    # Warmup
    sample(MersenneTwister(0), model, sampler, sampler isa ParallelMALASampler ? sampler.T : 100;
           progress=false, initial_params=x0)
    GC.gc()
    ...
    for _ in 1:reps
        ...
        t0 = time_ns()
        sample(MersenneTwister(42), model, sampler, N; progress=false, initial_params=x0)
        if x0 isa CUDA.CuArray
            CUDA.synchronize()
        end
...
@testset "GPU performance vs sequential CPU MALA" begin
    ...
    gpu_model = DensityModel(
        logp_gpu, gradlogp_gpu, D;
        hvp=hvp_gpu,
        logdensity_batch=logp_batch_gpu,
        grad_logdensity_batch=gradlogp_batch_gpu,
        hvp_batch=hvp_batch_gpu,
    )
    gpu_sampler = ParallelMALASampler(ε; T=T, maxiter=maxiter,
                                      tol_abs=1f-3, tol_rel=1f-2,
                                      damping=0.5f0)
