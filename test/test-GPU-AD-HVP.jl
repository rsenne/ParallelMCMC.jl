...
@testset "GPU AD-HVP: ParallelMALASampler runs without analytical HVP" begin
    ...
    raw = sample(MersenneTwister(42), model, sampler, n_samples;
                 initial_params=x0, progress=false)
...
@testset "GPU AD-HVP: matches CPU AD-HVP (same Mooncake path)" begin
    ...
    raw_gpu = sample(MersenneTwister(7), model_gpu, sampler, n_samples;
                     initial_params=CUDA.zeros(Float32, D), progress=false)
    β_gpu = vec(mean(reduce(hcat, [Array(s.x) for s in raw_gpu[(n_burn + 1):end]]); dims=2))
